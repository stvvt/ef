...
class core_Cls
{
    ...
    /**
     * Псевдоними на съществуващи класове
     *
     * @var array
     */
    ...
    public static function getClassRealName($alias)
    {
        $fullName = $alias;

        if (isset(static::$aliases[strtolower($alias)])) {
            $fullName = static::$aliases[strtolower($alias)];
        }
    ...
    {
        $realName = static::getClassRealName($className);

        if ($bStatus = static::load($realName)) {
            if (strcasecmp($realName, $className) !== 0) {
                class_alias($realName, $className);
            }
        }
...
spl_autoload_register(array('core_Cls', 'autoload'), true, true);
